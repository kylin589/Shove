...
function GetSchemeBonusScalec() {
    ...
    if (!isGetSchemeBonusScalec) {
        try {
            ...
            Lottery_BuySFC.GetSchemeBonusScalec(currentLotteryID, GetSchemeBonusScalec_callback);

            isGetSchemeBonusScalec = true;
        }
        catch (e) {
            time_GetSchemeBonusScalec = setTimeout("Lottery_BuySFC.GetSchemeBonusScalec(" + currentLotteryID + ",GetSchemeBonusScalec_callback);", 2000);
        }
    }
}
...
function GetServerTime(lotteryID) {
    ...
    try {
        ...
        Lottery_BuySFC.GetSysTime(GetServerTime_callback);
...
function GetAddone(lotteryID, IssueId) {
    ...
    try {
        ...
        Lottery_BuySFC.GetAddone(lotteryID, IssueId, GetAddone_callback);

    }
    catch (e) {

        time_GetAddone = setTimeout("Lottery_BuySFC.GetAddone(" + lotteryID + "," + IssueId + " ,GetAddone_callback);", 2000);
    }
...
function GetAddone_callback(response) {
    ...
    if (response == null || response.value == null) {
        ...
        time_GetAddone = setTimeout("Lottery_BuySFC.GetAddone(" + currentLotteryID + "," + currentLotteryID + ",GetAddone_callback);", 2000);
...
function PageEant() {
    ...
    $("input:radio[name='playType']").each(function() {
...
function GetIsuseInfo(lotteryID) {
    ...
    try {
        ...
        Lottery_BuySFC.GetIsuseInfo(lotteryID, GetIsuseInfo_callback);
...
function GetIsuseInfo_callback(response) {
   ...
   var arrcurrIsuse = currIsuse.split(',');
    ...
    lastIsuseInfo.innerHTML = arrcurrIsuse[3];
...
function clickPlayType(t) {
    ...
    switch (t) {
        case '7402':
            playTypeName = '复式';
            iframe_playtypes.location.href = '../Home/Room/playtypes/SFC/SFC_F.htm';
            break;
        ...
        case '74011':
            ...
            playTypeID = '7401';
            playTypeName = '单式';
            iframe_playtypes.location.href = '../Home/Room/playtypes/SFC/SFC_D.htm';
            break;
    ...
    if (t == '74011' || t == '74022') {
      ...
      $Id('iframe_playtypes').style.display = '';
      $Id('tagContent4').style.display = '';
      $Id('tagContent5').style.display = '';
    }
}
...
function btn_OK() {
    ...
    try {
        var LotteryNumber = Lottery_BuySFC.AnalyseScheme(document.getElementById("tbLotteryNumbers").value, document.getElementById('HidLotteryID').value, document.getElementById('tbPlayTypeID').value);
...
function Page_load(lotteryId, number) {
    ...
    clickPlayType('7401'); //加载
}


function newBuy() {
    $Id('playType101').checked = true;
