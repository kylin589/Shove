...
function GetSchemeBonusScalec() {
    ...
    if (!isGetSchemeBonusScalec) {
        try {
            ...
            Lottery_Buy22x5.GetSchemeBonusScalec(currentLotteryID, GetSchemeBonusScalec_callback);

        }
        catch (e) {
            time_GetSchemeBonusScalec = setTimeout("Lottery_Buy22x5.GetSchemeBonusScalec(" + currentLotteryID + ",GetSchemeBonusScalec_callback);", 2000);
        }
    }
}
...
function GetServerTime(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy22x5.GetSysTime(GetServerTime_callback);
...
function GetIsuseInfo(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy22x5.GetIsuseInfo(lotteryID, GetIsuseInfo_callback);
...
//判断是否隐藏机选按钮
function HidBtnRand(playTypeName) {
    if (playTypeName.indexOf("复式") > -1) {
        $Id("random1").disabled = "false";
        $Id("random5").disabled = "false";
        $Id("random1").style.cursor = '';
        $Id("random5").style.cursor = '';
    }
    else {
        $Id("random1").disabled = "";
        $Id("random5").disabled = "";
        $Id("random1").style.cursor = 'pointer';
        $Id("random5").style.cursor = 'pointer';
    }
}
...
function newBuy(lotteryID) {
    ...
    clickPlayType(901);
    $Id("playType901").checked=true;
    $Id("playType902").checked=false;
...
function schemeAll(lotteryID) {
    ...
    $Id("iframeSchemeAll").src = "../Home/Room/SchemeAll.aspx?Radom=" + Math.random()+1 + "&LotteryID=" + lotteryID + "&IsuseID=" + $Id("HidIsuseID").value;
    $Id("trGoon").style.display = "none";
    $Id("trShowJion").style.display = "none"; 
    $Id("tagContent4").style.display = "none"; 
    $Id("tagContent5").style.display = "none"; 
