...
function GetSchemeBonusScalec() {
    ...
    if (!isGetSchemeBonusScalec) {
        try {
            ...
            Lottery_BuyJQC.GetSchemeBonusScalec(currentLotteryID, GetSchemeBonusScalec_callback);

            isGetSchemeBonusScalec = true;
        }
        catch (e) {
            time_GetSchemeBonusScalec = setTimeout("Lottery_BuyJQC.GetSchemeBonusScalec(" + currentLotteryID + ",GetSchemeBonusScalec_callback);", 2000);
        }
    }
}
...
function GetServerTime(lotteryID) {
    ...
    try {
        ...
        Lottery_BuyJQC.GetSysTime(GetServerTime_callback);
...
function GetAddone(lotteryID, IssueId) {
    ...
    try {
        ...
        Lottery_BuyJQC.GetAddone(lotteryID, IssueId, GetAddone_callback);

    }
    catch (e) {

        time_GetAddone = setTimeout("Lottery_BuyJQC.GetAddone(" + lotteryID + "," + IssueId + " ,GetAddone_callback);", 2000);
    }
...
function GetAddone_callback(response) {
    ...
    if (response == null || response.value == null) {
        ...
        time_GetAddone = setTimeout("Lottery_BuyJQC.GetAddone(" + currentLotteryID + "," + currentLotteryID + ",GetAddone_callback);", 2000);
...
function GetIsuseInfo(lotteryID) {
    ...
    try {
        ...
        Lottery_BuyJQC.GetIsuseInfo(lotteryID, GetIsuseInfo_callback);
...
function GetIsuseInfo_callback(response) {
    ...
    var st=lastIsuse.split(',');
   
    lastIsuseInfo.innerHTML = st[0];
    $Id("LastWinNumber").innerHTML = st[1]; //开奖号码
...
function clickPlayType(t) {
        ...
        case '202':
            playTypeName = '复式';
            iframe_playtypes.location.href = '../Home/Room/playtypes/JQC/JQC_F.htm';
            break;
        ...
        case '2011':
            ...
            playTypeID = '201';
            playTypeName = '单式';
            iframe_playtypes.location.href = '../Home/Room/playtypes/JQC/JQC_D.htm';
            break;
...
function btn_OK() {
    ...
    try {
        var LotteryNumber = Lottery_BuyJQC.AnalyseScheme(document.getElementById("tbLotteryNumbers").value, document.getElementById('HidLotteryID').value, document.getElementById('tbPlayTypeID').value);
