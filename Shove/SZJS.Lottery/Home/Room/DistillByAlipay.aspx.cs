...
public partial class Home_Room_DistillByAlipay : SitePageBase
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (_User != null)
            {
                ...
                if (HidIsCps.Value == "1")
                {
                    DataTable dt = Shove.Database.MSSQL.Select("select BonusAllow from T_Users where ID = " + _User.ID.ToString() + "");
                    ...
                    lbMoney.Text = dt.Rows[0]["BonusAllow"].ToString();
                }
                else
                {
