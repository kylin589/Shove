...
function GetServerTime(lotteryID) {
    ...
    try {
        Lottery_Index_BuyCQSSC.GetSysTime(GetServerTime_callback);
    }
    catch (e) {
        //如果失败了，就继续读取
...
function GetIsuseInfo(lotteryID) {
    currentLotteryID = lotteryID;
    try {
        Lottery_Index_BuyCQSSC.GetIsuseInfo(lotteryID, GetIsuseInfo_callback);
    }
    catch (e) {
...
function GetLuckNumber(lotteryID) {
    var v = Lottery_Index_BuyCQSSC.GenerateLuckLotteryNumber(lotteryID).value;
    if (v.split("|").length < 2) {
    } else {
     ...
     if ($Id("toCurrIsuseEndTime").innerHTML == "本期已截止投注" < 0)
      ...
      var o_list_LotteryNumber= $Id("list_LotteryNumber");
       ...
       $Id("Multiple").value=tb_Multiple;
        ...
        if (!confirm(TipStr + "按“确定”即表示您已阅读《时时彩投注协议》并立即提交方案，确定要提交方案吗？"))
        {
           return false;
        }
        __doPostBack('btn_OK', '');
