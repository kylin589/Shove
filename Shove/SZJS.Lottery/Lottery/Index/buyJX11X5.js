...
function GetServerTime(lotteryID) {
    ...
    try {
        Lottery_Index_BuyJX11X5.GetSysTime(GetServerTime_callback);
    }
    catch (e) {
        //如果失败了，就继续读取
...
function showNextIsuseTime(lotteryID, tPoor) {
    ...
    if (IsuseStartTime == null) {
        IsuseStartTime = new Date(Lottery_Index_BuyJX11X5.GetNextIsuseInfo(lotteryID).value.replace(new RegExp("-", "g"), "/"));
    }
...
function GetIsuseInfo(lotteryID) {
    currentLotteryID = lotteryID;
    try {
        Lottery_Index_BuyJX11X5.GetIsuseInfo(lotteryID, GetIsuseInfo_callback);
    }
    catch (e) {
...
function GetLuckNumber(lotteryID) {
    var v = Lottery_Index_BuyJX11X5.GenerateLuckLotteryNumber(lotteryID).value;
    if (v.split("|").length < 2) {
    } else {
...
function BallMoving() {
    if (isMove) {
        for (var i = 1; i < number.length; i++) {
            $Id("tdLuckNumber" + i.toString()).value =GetBallNum(number[GetRandNumber(number.length-1)]);
        }
    }
}
...
function BindLuckNumber() {
    ...
    var Num="";
    for (var i = 1; i < number.length; i++) {
        $Id("tdLuckNumber" + i.toString()).value =GetBallNum(number[i]);
        Num+=" "+GetBallNum(number[i]);
    }
      var customOptions = document.createElement("OPTION");
  ...
  function btn_2_RandManyClick(n) {     
    ...
    for (var j = 1; j <= n; j++) {  
        ...
        var prt="";
        for(var i=0;i<5;){
            t = parseInt(Math.random()*count+1);
     ...
     if ($Id("toCurrIsuseEndTime").innerHTML == "本期已截止投注" < 0)
      {
       ...
       $Id("Multiple").value=tb_Multiple;
        ...
        if (!confirm(TipStr + "按“确定”即表示您已阅读《11选5投注协议》并立即提交方案，确定要提交方案吗？"))
        {
           return false;
        }
        __doPostBack('btn_OK', '');
