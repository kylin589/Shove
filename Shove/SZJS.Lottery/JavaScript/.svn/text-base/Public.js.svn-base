...
(function($) {
    $.fn.fadeImg = function(settings) {
        ...
        this[0].style.filter = "progid:DXImageTransform.Microsoft.Fade(duration=1.0,overlap=1.0)";
        setInterval(function() {
            if (index >= settings.imgs.length)
                index = 0;
            else
                index++;
            jqObj[0].filters[0].Apply();
            jqObj.attr("src", settings.imgs[index]);
            jqObj[0].filters[0].play();
        }, settings.speed);
        return jqObj;
    }
})(jQuery)
