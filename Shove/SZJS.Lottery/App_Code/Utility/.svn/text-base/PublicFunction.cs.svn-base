...
public class PF
{
    ...
    public static string AlipayAccountRegisterPid = Shove._Web.WebConfig.GetAppSettingsString("AlipayAccountRegisterPid");
    public static double AlipayRegDonateMoney = Shove._Web.WebConfig.GetAppSettingsDouble("AlipayRegDonateMoney", 0);
    ...
    public static bool GetZCDCNoIsOverdue(long IsuseID, string No)
    {
        ...
        DataTable dt = new DAL.Views.V_IsuseForZCDC().Open("DateTime", "[IsuseID]=" + IsuseID.ToString() + " and [No]=" + No, "");
        ...
        if (DateTime.Now > Shove._Convert.StrToDateTime(dt.Rows[0][0].ToString(), DateTime.Now.ToString()))
        {
            IsOverdue = true;
        }
    ...
    public static bool GetZCDCSchemesIsOverdue(long IsuseID, string No)
    {
        ...
        if (DateTime.Now > (Shove._Convert.StrToDateTime(dt.Rows[0][0].ToString(), DateTime.Now.ToString()).AddMinutes(30)))
        {
            IsOverdue = true;
        }
    ...
    {
        new Log("System").Write("--------------卡密" + dt.Rows.Count.ToString());
    ...
    public static DataTable GetDataTable(DataTable dt, int Type2)
    {
        ...
        if (Type2 == 3)
        {
            Rows = dt.Select("LotteryType2 = " + Type2 + "and LotteryID <> " + SZJS.Lottery.ZCDC.ID.ToString(), "EndTime desc");
        }
        else
        {
        ...
        foreach (DataRow dr in Rows)
        {
            ...
            dr1[2] = dr[2].ToString();
            dr1[3] = dr[3].ToString();
            dr1[4] = new SZJS.Lottery()[int.Parse(dr[1].ToString())].ShowNumber(dr[4].ToString());
            dr1[5] = dr[5].ToString();
            dr1[6] = dr[6].ToString();
            dr1[7] = dr[7].ToString();
    ...
    public static DataTable GetZCDCBuyContent(string BuyNum, long SchemeID, ref string vote)
    {
        ...
        bool IsGetScheme = new SZJS.Lottery()[SZJS.Lottery.ZCDC.sID].GetSchemeSplit(lotteryNumber, ref BuyContent, ref vote);
        ...
        for (int i = 0; i < Team.Length; i++)
        {
            ...
            string LetBall = "";
            string sp = "";
            if (PlayType == SZJS.Lottery.ZCDC.PlayType_SPF.ToString())
            {
            ...
            if (PlayType == SZJS.Lottery.ZCDC.PlayType_ZJQ.ToString())
            {
            ...
            if (PlayType == SZJS.Lottery.ZCDC.PlayType_SXDS.ToString())
            {
            ...
            if (PlayType == SZJS.Lottery.ZCDC.PlayType_ZQBF.ToString())
            {
            ...
            if (PlayType == SZJS.Lottery.ZCDC.PlayType_BQCSPF.ToString())
            {
    ...
    */
    ...
    //2009.12.22 用户注册机器码信息
    public static string GetMID()
    {
        SqlConnection conn = Shove.Database.MSSQL.CreateDataConnection<System.Data.SqlClient.SqlConnection>();
        ...
        string MID = Shove._System.SystemInformation.GetNetCardMACAddress() + DatabaseName;

        return Shove._Security.Encrypt.NoUnEncryptString(GetCallCert(), MID, "MID-", 5, 5, 0);
    }
    ...
    public static bool IsSystemRegister()
    {
        ...
        if (System.Web.HttpContext.Current.Application[CacheKey.IsSystemRegister] != null)
        {
            return Shove._Convert.StrToBool(System.Web.HttpContext.Current.Application[CacheKey.IsSystemRegister].ToString(), false);
        }
        else
        {
            string MID = "MID-20800-81207-05198-94925-80974"; //GetMID();
            SystemOptions so = new SystemOptions();
            if (so["SerialNumberForShoveSoft_MHB"].Value.ToString() != Shove._Security.Encrypt.NoUnEncryptString(GetCallCert(), MID, "SZJS-", 5, 5, 0))
            {
                return false;
            }
            return true;
