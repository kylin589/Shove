...
namespace SZJS.ElectronTicketbase.Task
{
    ...
    /// 江西时时彩、双色球、七乐彩、3D、
    /// </summary>
    public class ElectronTicket_FC
    {
        ...
        private Message msg = new Message("ElectronTicketFC");
        private Log log = new Log("ElectronTicketFC");

        public int State = 0;   // 0 停止 1 运行中 2 置为停止

        public string ElectronTicketbase_FC_Getway;//网关地址
        public string ElectronTicketbase_FC_Agent_UserNumber;//代理商编号
        public string ElectronTicketbase_FC_Agent_Key;//代理商密钥
        /// <summary>
        /// 福彩104
        /// </summary>
        /// <param name="connectionstring"></param>
        public ElectronTicket_FC(string connectionstring)
        {
        ...
        public void Run()
        {
            ...
            if ((ElectronTicketbase_FC_Getway == "") || (ElectronTicketbase_FC_Agent_UserNumber == "") || (ElectronTicketbase_FC_Agent_Key == ""))
            {
                log.Write("ElectronTicket_FC Task 参数配置不完整.");
            ...
            lock (this) // 确保临界区被一个 Thread 所占用
            {
                ...
                log.Write("ElectronTicket_FC Task Start.");
            }
        }
        ...
        public void Do()
        {
            while (true)
            {
                if (State == 2)
                {
                    msg.Send("ElectronTicket_FC Task Stop.");
                    log.Write("ElectronTicket_FC Task Stop.");
        ...
        {
            DataTable dt = new DAL.Views.V_SchemeSchedules().Open(ConnectionString, " ID, LotteryID, PlayTypeID, LotteryNumber, Multiple, Money", "Buyed = 0 and PrintOutType = 104 and (GetDate() between StartTime and EndTime) and Schedule >= 100 and not [ID] in (select SchemeID from T_SchemesSendToCenter)", "[ID]");
            if (dt == null)
            {
