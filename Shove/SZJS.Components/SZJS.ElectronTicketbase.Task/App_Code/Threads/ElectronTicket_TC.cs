...
{
    /// <summary>
    /// 大乐透、排三、排五、七星彩、22选5、胜负彩、四场进球、六场半、任九场
    /// </summary>
    public class ElectronTicket_TC
    {
        ...
        private Message msg = new Message("ElectronTicketTC");
        private Log log = new Log("ElectronTicketTC");

        public int State = 0;   // 0 停止 1 运行中 2 置为停止

        public string ElectronTicketbase_TC_Getway;//网关地址
        public string ElectronTicketbase_TC_Agent_UserNumber;//代理商编号
        public string ElectronTicketbase_TC_Agent_Key;//代理商密钥
        /// <summary>
        /// 体彩103
        /// </summary>
        /// <param name="connectionstring"></param>
        public ElectronTicket_TC(string connectionstring)
        {
        ...
        public void Run()
        {
            ...
            if ((ElectronTicketbase_TC_Getway == "") || (ElectronTicketbase_TC_Agent_UserNumber == "") || (ElectronTicketbase_TC_Agent_Key == ""))
            {
                log.Write("ElectronTicket_TC Task 参数配置不完整.");
            ...
            lock (this) // 确保临界区被一个 Thread 所占用
            {
                ...
                log.Write("ElectronTicket_TC Task Start.");
            }
        }
        ...
        public void Do()
        {
            while (true)
            {
                if (State == 2)
                {
                    msg.Send("ElectronTicket_TC Task Stop.");
                    log.Write("ElectronTicket_TC Task Stop.");
        ...
        private void Send()
        {
            DataTable dt = new DAL.Views.V_SchemesSendToCenter().Open(ConnectionString, "", "Buyed = 0 and PrintOutType = 103 and (GetDate() between StartTime and EndTime) and ((Sends = 0) or (Sends < 3 and HandleResult = 0 and datediff(minute, [Datetime], GetDate()) > 3))", "[ID]");
            ...
            for (int i = 0; i < SendCount; i++)
            {
                ...
                for (int j = i * 500; j < (i * 500) + 500; j++)
                {
                    ...
                    try
                    {
                        ...
                        wParamStr += @"<msg>";
                        wParamStr += @"<head transcode='" + transcode + "' partnerid='" + ElectronTicketbase_TC_Agent_UserNumber + "' version='1.0' time='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "'/>";
                        wParamStr += @"<body>";
                    ...
                    try
                    {
                        wSignStr = Shove._Security.Encrypt.MD5(wParamStr + ElectronTicketbase_TC_Agent_Key, "UTF-8").ToUpper();
                    }
                    catch (Exception ex)
                    {
                        msg.Send(ex.Message);
                    ...
                    {
                         log.Write(wParamStr);
                        ReceiveString = PublicFunction.Post(ElectronTicketbase_TC_Getway, wParamStr, wSignStr);
                    }
                    catch
                    {
