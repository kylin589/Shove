...
function GetSchemeBonusScalec() {
    ...
    if (!isGetSchemeBonusScalec) {
        try {
            ...
            Lottery_BuyLCBQC.GetSchemeBonusScalec(currentLotteryID, GetSchemeBonusScalec_callback);

            isGetSchemeBonusScalec = true;
        }
        catch (e) {
            time_GetSchemeBonusScalec = setTimeout("Lottery_BuyLCBQC.GetSchemeBonusScalec(" + currentLotteryID + ",GetSchemeBonusScalec_callback);", 2000);
        }
    }
}
...
function GetServerTime(lotteryID) {
    ...
    try {
        ...
        Lottery_BuyLCBQC.GetSysTime(GetServerTime_callback);
...
function GetAddone(lotteryID, IssueId) {
    ...
    try {
        ...
        Lottery_BuyLCBQC.GetAddone(lotteryID, IssueId, GetAddone_callback);

    }
    catch (e) {

        time_GetAddone = setTimeout("Lottery_BuyLCBQC.GetAddone(" + lotteryID + "," + IssueId + " ,GetAddone_callback);", 2000);
    }
...
function GetAddone_callback(response) {
    ...
    if (response == null || response.value == null) {
        ...
        time_GetAddone = setTimeout("Lottery_BuyLCBQC.GetAddone(" + currentLotteryID + "," + currentLotteryID + ",GetAddone_callback);", 2000);
...
function GetIsuseInfo(lotteryID) {
    ...
    try {
        ...
        Lottery_BuyLCBQC.GetIsuseInfo(lotteryID, GetIsuseInfo_callback);
...
function clickPlayType(t) {
    ...
    switch (t) {
        case '1502':
            playTypeName = '复式';
            iframe_playtypes.location.href = '../Home/Room/playtypes/LCBQC/LCBQC_F.htm';
            break;
        ...
        case '15011':
            ...
            playTypeID = '1501';
            playTypeName = '单式';
            iframe_playtypes.location.href = '../Home/Room/playtypes/LCBQC/LCBQC_D.htm';
            break;
...
function btn_OK() {
    ...
    try {
        var LotteryNumber = Lottery_BuyLCBQC.AnalyseScheme(document.getElementById("tbLotteryNumbers").value, document.getElementById('HidLotteryID').value, document.getElementById('tbPlayTypeID').value);
...
function Page_load(lotteryId, number) {
    ...
    clickPlayType('1501'); //加载
}

function newBuy() {
    $Id("playType1501").checked = true;
